...
state={"running":False,"meta_connected":False,"username":"","last_error":"","last_check":"","last_published":"","preview":{},"queue":[],"next_publish_at":0,"meta_diagnostic":"","quota_usage":None,"quota_total":None,"quota_duration":None}
...
def publishing_limit(t,u,quiet=False):
 try:
  ...
  return None
def quota_full(t,u):
 d=publishing_limit(t,u,True)
 if not d:return False
 total=state.get("quota_total");used=state.get("quota_usage");return total is not None and total>0 and used is not None and used>=total
...
def normalize_title(x):
 x=unicodedata.normalize("NFKD",clean(x)).encode("ascii","ignore").decode().lower();x=re.sub(r"\s+[2-9]\s*$","",x);return re.sub(r"[^a-z0-9]+"," ",x).strip()
...
def loop():
 ...
 c=load_config();interval=max(60,int(c.get("check_interval") or 60));token=str(c.get("meta_access_token") or "");uid=str(c.get("meta_ig_user_id") or DEFAULT_IG_USER_ID);rss=str(c.get("rss_url") or DEFAULT_RSS);next_pub=0;quota_log=False
 try:
  ...
  while not stop_event.is_set():
   ...
   if item and now>=due:
    if quota_full(token,uid):
     next_pub=time.time()+QUOTA_RECHECK;state["next_publish_at"]=next_pub
     if not quota_log:log(f"🛑 Quota Meta raggiunta: {state['quota_usage']}/{state['quota_total']}. Nessun tentativo di pubblicazione; nuovo controllo tra {QUOTA_RECHECK//60} minuti.");quota_log=True
    ...
    try:
     ...
     with store_lock:s=load_store();s["queue"]=[x for x in s["queue"] if x.get("link")!=item["link"]];set_last(item["link"]);remember_title(s,item["title"]);stat_inc(s,"published");s["rate_limit_level"]=0;s["cooldown_until"]=0;save_store(s);sync_state(s);has_more=bool(s["queue"])
     state["last_published"]=item["title"];state["last_error"]="";log(f"✅ Pubblicato via API Meta. Media ID: {mid}");publishing_limit(token,uid,True);next_pub=time.time()+PUBLISH_GAP;state["next_publish_at"]=next_pub if has_more else 0
    except Exception as e:
...
def clear_queue():
 with store_lock:
  s=load_store();n=len(s.get("queue",[]));s["queue"]=[];s["cooldown_until"]=0;s["rate_limit_level"]=0;save_store(s);sync_state(s)
 with lock:state["next_publish_at"]=0
 log(f"🗑️ Coda cancellata manualmente: rimossi {n} articoli.");return jsonify({"ok":True,"removed":n})
@app.post("/test_waha")
def test_waha():save_config(formcfg());waha("Test Montagne & Paesi: Instagram Bot v2.2.5 funzionante.");return redirect("/")
@app.get("/status")
def status():
...
if __name__=="__main__":log(f"🟢 Web UI pronta. Versione {APP_VERSION}.");log("🛡️ Quota Meta automatica, diagnostica e gestione coda attive.");app.run(host="0.0.0.0",port=8080)
