"""Montagne & Paesi Instagram Bot v2.3.6 - smart queue, resilient fetch, strict semantic hashtags."""
import re,time,unicodedata
from collections import Counter
import feedparser
import meta_v2 as core

APP_VERSION="2.3.6";MAX_AGE_HOURS=18;MAX_QUEUE=60;FETCH_RETRY=300
...
def richer_hashtags(title,body):
 title_raw=core.clean(title);lead_raw=core.clean((body or "")[:1000]);focus_raw=(title_raw+" "+lead_raw).strip();focus=norm(focus_raw);tags=[]
 def add(tag):
 ...
 geo=[(r"\bbusto arsizio\b",["BustoArsizio","Varese","Lombardia"]),(r"\bvarese\b",["Varese","Lombardia"]),(r"\bbergamo\b",["Bergamo","Bergamasca","Lombardia"]),(r"\bbrescia\b",["Brescia","Bresciano","Lombardia"]),(r"\bfino del monte\b",["FinoDelMonte","ValSeriana","Orobie"]),(r"\bclusone\b",["Clusone","ValSeriana","Orobie"]),(r"\bgandino\b",["Gandino","ValGandino","ValSeriana"]),(r"\bval ?seriana\b",["ValSeriana","Orobie","Bergamo"]),(r"\bval ?brembana\b",["ValBrembana","Orobie","Bergamo"]),(r"\bval ?camonica\b",["ValCamonica","Brescia","Lombardia"]),(r"\bfranciacorta\b",["Franciacorta","Brescia","Lombardia"]),(r"\biseo\b",["Iseo","LagoDIseo","Brescia"]),(r"\borobie\b",["Orobie","Montagna","Lombardia"]),(r"\bbologna\b",["Bologna","EmiliaRomagna"]),(r"\bmilano\b",["Milano","Lombardia"]),(r"\broma\b",["Roma","Lazio"]),(r"\bgermania\b",["Germania"])]
 for p,vals in geo:
  if hf(p):
   for v in vals:add(v)
 named=[(r"\btre valli varesine(?: women's race| women.?s race)?\b",["TreValliVaresine"]),(r"\bdario acquaroli\b",["DarioAcquaroli"]),(r"\borobie cup(?: junior)?\b",["OrobieCup"]),(r"\bliceo minghetti\b",["LiceoMinghetti"]),(r"\bwunstorf\b",["Wunstorf"]),(r"\b(?:universita|università) (?:degli studi )?di bergamo\b|\bunibg\b",["UniBg"])]
 for p,vals in named:
  if hf(p):
   for v in vals:add(v)
 themes=[(r"\bscuol\w*\b|\bliceo\b|\bstudent\w*\b",["Scuola","Studenti"]),(r"\bistruzion\w*\b",["Istruzione"]),(r"\bprotest\w*\b|\bmanifestazion\w*\b",["Protesta"]),(r"\bdrone\w*\b",["Drone"]),(r"\bbase (?:aerea|militare)\b|\baeroporto militare\b",["BaseMilitare"]),(r"\bforze armate\b|\besercito\b|\baeronautica militare\b",["ForzeArmate"]),(r"\bmtb\b|\bmountain bike\b",["MTB","MountainBike"]),(r"\bciclism\w*\b|\bfederazione ciclistica\b",["Ciclismo"]),(r"\bcalcio\b",["Calcio"]),(r"\bbasket\b|\bpallacanestro\b",["Basket"]),(r"\bpallavolo\b|\bvolley\b",["Pallavolo"]),(r"\bsci\b|\bsciator\w*\b",["Sci"]),(r"\bnuoto\b|\bnuot\w*\b",["Nuoto"]),(r"\bincidente\b|\bschianto\b",["Cronaca","Incidente","SicurezzaStradale"]),(r"\barrest\w*\b|\bdenunc\w*\b|\bspaccio\b|\bdroga\b",["Cronaca","Sicurezza"]),(r"\bmaltempo\b|\btemporale\w*\b|\bfrana\b|\balluvion\w*\b",["Maltempo","Meteo"]),(r"\bincendio\b|\bvigili del fuoco\b",["Cronaca","VigiliDelFuoco"]),(r"\bfesta\b|\bfestival\b|\bsagra\b",["Eventi"]),(r"\bmontagna\b|\brifugio\b|\balpeggio\b",["Montagna","Natura"]),(r"\bministro\b|\bministero\b|\bparlamento\b|\bconsiglio comunale\b",["Politica"])]
 for p,vals in themes:
  if hf(p):
   for v in vals:add(v)
 # Aeroporto e Orio al Serio sono indipendenti: mai piu OrioAlSerio per un generico aeroporto.
 if hf(r"\borio al serio\b"):add("OrioAlSerio")
 if hf(r"\baeroporto\b") and not hf(r"\baeroporto militare\b|\bbase (?:aerea|militare)\b"):add("Aeroporto")
 # Contesto internazionale e sicurezza solo quando il lead lo giustifica.
 if hf(r"\bgermania\b|\bfrancia\b|\bspagna\b|\bregno unito\b|\bucraina\b|\brussia\b|\bstati uniti\b") and hf(r"\bdrone\w*\b|\bmilitar\w*\b|\bpolizia\b|\bforze armate\b|\bguerra\b|\battacco\b"):add("CronacaInternazionale")
 if hf(r"\bpolizia\b|\bmilitar\w*\b|\bforze armate\b|\bbase (?:aerea|militare)\b"):add("Sicurezza")
 stop_first={"La","Il","Lo","Gli","Le","Un","Una","Uno","Nel","Nella","Nelle","Nei","Al","Alla","Alle","A","Da","Dal","Dalla","Di","Del","Della","Dei","Delle","Per","Con","Tra","Fra","Oggi","Ieri","Domani","Martedi","Mercoledi","Giovedi","Venerdi","Sabato","Domenica","Adnkronos","Allerta"}
 candidates=[]
 for m in re.finditer(r"(?<!\w)([A-ZÀ-Ý][A-Za-zÀ-ÿ0-9'’.-]+(?:\s+(?:di|del|della|dei|degli|delle|e|[A-ZÀ-Ý][A-Za-zÀ-ÿ0-9'’.-]+)){0,3})",focus_raw):
  phrase=re.sub(r"\s+"," ",m.group(1)).strip(" .,:;–—-\"");words=phrase.split()
  if not words or words[0] in stop_first:continue
  if len(words)==1 and len(words[0])<5:continue
  candidates.append(phrase)
 counts=Counter(c.lower() for c in candidates);used=0
 for phrase in candidates:
  if used>=3:break
  low=phrase.lower()
  if low in {"montagne e paesi","web info","allerta"}:continue
  if low in norm(title_raw) or counts[low]>=2:
   add("".join(w.capitalize() for w in phrase.split()));used+=1
 sport=hf(r"\b(?:sport|sportiv[oaie]|gara|gare|campionat[oi]|campion[ei]|campionessa|campionesse|torneo|coppa|cup|race|mtb|mountain bike|ciclism\w*|calcio|basket|pallacanestro|pallavolo|volley|sci|nuoto)\b")
 event=hf(r"\b(?:gara|gare|campionato|torneo|coppa|cup|race|manifestazione|competizione)\b")
 if sport:add("Sport")
...
try:
 ...
 if removed:core.log(f"🧹 Migrazione coda v2.3.6: rimossi {removed} articoli automatici/vecchi.")
except Exception as e:core.log(f"⚠️ Migrazione coda intelligente non riuscita: {e}")
if __name__=="__main__":
 core.log(f"🟢 Web UI pronta. Versione {APP_VERSION}.");core.log("🧠 Hashtag semantici rigorosi: associazioni false eliminate + contesto internazionale attivo.");core.app.run(host="0.0.0.0",port=8080)
